...
def game():
    ...
    #defining funtion for results of test
    def check_result():
        if entry.get() == words[word]:

            #here start time when window is opened
            #and time when window is destroyed
            end = timer()

            #we deduct the start time from the end
            #time and calculate results using timeit
            #function
            print(end-start)
        else:
            print("wrong input")
